            ...
            <div>
              ...
              {fabrics.length === 0 ? (
                ...
                <div className="space-y-4 max-h-[320px] overflow-y-auto pr-1">
                  {fabrics.map((f) => (
                    <div key={f.id}>
                      <p className="text-[11px] font-bold text-slate-500 mb-2">{f.name}</p>
                        ...
                        {f.colors.map((c) => {
                          ...
                          return (
                            <button
                              ...
                              className={`relative aspect-square rounded-xl overflow-hidden border-2 transition-all ${active ? 'border-[#74AE2C] scale-105 shadow-md' : 'border-white hover:border-slate-200'}`}
                              style={{ backgroundColor: c.color_hex || '#ddd' }}
                            >
                              {c.image_url && (
                                <img src={c.image_url} alt={c.name} className="w-full h-full object-cover" crossOrigin="anonymous" />
                              )}
                              {active && (
